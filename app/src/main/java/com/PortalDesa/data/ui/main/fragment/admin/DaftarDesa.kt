...
import kotlinx.android.synthetic.main.activity_daftar_desa.*

class DaftarDesa : Fragment(), View.OnClickListener {


    override fun onCreateView(inflater: LayoutInflater,
                              container: ViewGroup?,
                              savedInstanceState: Bundle?): View? {
        return inflater.inflate(R.layout.activity_daftar_desa, container, false)
    }
    ...
    private fun tabSelected(position: Int) {
        if (position == 1) {
            ...
            (context as MainActivityAdmin).showFragmentAllowingStateLoss(DaftarAkunFragment(), R.id.fragment_container)
        } else {
            ...
            (context as MainActivityAdmin).showFragmentAllowingStateLoss(DaftarAkunAdminFragment(), R.id.fragment_container)
        }
    }
}
